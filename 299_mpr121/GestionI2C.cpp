...
bool GestionI2C::write8(uint8_t data) {
	if(wiringPiI2CWrite (handle, data) >= 0) {
		return true;
	} else {
		return false;
	}
...
bool GestionI2C::write8(uint8_t reg, uint8_t data) {
	if(wiringPiI2CWriteReg8 (handle, reg, data) >= 0) {
		return true;
	} else {
		return false;
	}
...
bool GestionI2C::write16(uint8_t reg, uint16_t data) {
	if(wiringPiI2CWriteReg16 (handle, reg, data) >= 0) {
		return true;
	} else {
		return false;
	}
...
bool GestionI2C::write16Swapped(uint8_t reg, uint16_t val) {
	...
	data.uSData = val;
	swapU(data);
	if(wiringPiI2CWriteReg16 (handle, reg, data.uSData) >= 0) {
		return true;
	} else {
		return false;
	}
...
// **********************************************************
// Lecture de plusieurs valeurs en une fois
// Très utile pour avoir des valeurs cohérentes sur un module
// @Param le registre de depart
// @Param le nombre de valeurs
// @return la list de valeurs, une liste vide en cas d'erreur
// **********************************************************
std::list<int8_t> GestionI2C::readList(uint8_t reg, uint8_t nbVal) {
	std::list<int8_t> lstData;
	char tabReg[1] = {reg};
	write(handle, tabReg, 1);
	char data[nbVal] = {0};
	if(read(handle, data, nbVal) != nbVal) {
		return lstData;
	}
	for(int compteur = 0; compteur != nbVal; compteur++) {
		lstData.push_back((int8_t)data[compteur]);
	}
