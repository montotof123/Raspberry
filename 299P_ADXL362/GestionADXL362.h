...
class GestionADXL362 : GestionSpi {
	public:
		enum LinkLoop {
		...
		GestionADXL362(void);
		GestionADXL362(Channel);
		bool initADXL362(Channel);
	...
	private:
		...
		char spiData16[4];
		char fifoData[513];
		uint8_t genericRead8(uint8_t, const char*);
		uint16_t genericRead16(uint8_t, const char*);
		uint8_t genericWrite8(uint8_t, uint8_t, const char*);
		uint8_t genericWrite16(uint8_t, uint16_t, const char*);
